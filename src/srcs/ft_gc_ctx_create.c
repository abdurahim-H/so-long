...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/05/31 21:39:49 by abhudulo          #+#    #+#             */
/*   Updated: 2024/05/31 21:39:51 by abhudulo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_gc_ctx_manage(t_ctx *ctx, t_ptr p, void (*free_func)(t_ptr ))
{
	...
	if (free_func == NULL)
		free_func = &free;
	unman_data = calloc(1, sizeof(t_unman));
	if (unman_data == NULL)
		return (-1);
	unman_data->addr = p;
